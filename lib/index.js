...
async function sendAmqpMessages(ctx) {
  ...
  const messageOpts = { appId: "aupAppId", type: "messageType", userId: "publisher", headers: { "header1": "value" } };
  amqp.connect(url, connectionOpts, function (error0, connection) {
    ...
    connection.createChannel(async function (error1, channel) {
      ...
      while (running) {
        ...
        channel.sendToQueue(queue.name, Buffer.from(lines[i]), messageOpts);
        await sleep(rate);
        if (i === lines.length - 1) {
          i = 0;
        }
